...
[disk]
    comment = COMMENT
    # 配置共享是否能被浏览. (例如你从Windows的"网络"界面扫描到了一个主机, 点进去输密码后就是该主机上配置的分享, 如果配置为no, (默认yes), 别人就能看到主机上的分享. 如果主机上没有配置wsdd2服务, 这项可忽略, 反正别人也扫不到你)
    #browseable = no
    path = /root/test
	writable = yes
	# 权限相关
...
```
## KSMBD权限问题
所有rwx操作均是以登录用户的权限进行的, 如果登录的用户不存在于系统中, 则是用其他用户的权限(___---rwx), 匿名用户默认是其他用户<br>
```
